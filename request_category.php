    ...
    if (isset($_POST['submitCategoryRequest'])) {
        ...
        if (!$queryIfRequestedRes->num_rows && !$queryIfExistsRes->num_rows) {
            ...
            if ($queryInsertNewRequestRes) {
                ...
                echo "Greška pri prijavljivanju kategorije.";
                echo "</div> </div>";
            }
        } else {
    ...
    <div class="container">
        <div class="wrapper">
            <div class="loginRegisterRedirect" style="justify-content: center;">
                <button class="loginRegisterRedirectButton" onclick="location.href = 'pocetna.php';">Vrati se na početnu stranicu</button>
            </div>
            <form action="request_category.php" method="post" enctype="multipart/form-data">
                <input type="text" name="categoryName" id="categoryName" required placeholder="Ime kategorije" class="inputLogRes"><br>
                <textarea name="description" id="description" rows="4" required placeholder="Objasni kategoriju" class="inputLogRes"></textarea><br>
                <input class="startPageBttn" type="submit" value="Prijavi kategoriju" name="submitCategoryRequest" style="width:100%; padding: 10px 10px;">
            </form>
        </div>
    </div>
</body>
